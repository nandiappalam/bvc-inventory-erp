...
const app = express()
// AI Studio / Production allows PORT override, defaults to 3001 for clean local frontend dev proxying
...
app.get('*', (req, res) => {
  ...
  res.status(200).send(`
    <!DOCTYPE html>
    <html>
      <head><title>BVC ERP API Server</title></head>
      <body style="font-family: system-ui, sans-serif; padding: 40px; text-align: center; background: #f8fafc; color: #1e293b;">
        <h2>BVC ERP Backend Server is Running (Port ${PORT})</h2>
        <p style="color: #64748b; margin-bottom: 20px;">The frontend development server is accessible at <a href="http://localhost:3000" style="color: #2563eb; font-weight: bold;">http://localhost:3000</a></p>
        <p style="font-size: 13px; color: #94a3b8;">For Tauri / Desktop, launch using <code>npm run tauri dev</code></p>
      </body>
    </html>
  `);
});
...
const server = app.listen(PORT, '0.0.0.0', async () => {
  ...
  try {
    ...
    if (isPg && typeof db.ensurePostgresMasterSchema === 'function') {
