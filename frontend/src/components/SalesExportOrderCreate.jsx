...
/**
 * SalesExportOrderCreate - Sales Export Order Creation
 */
const SalesExportOrderCreate = () => {
    ...
    const loadData = async () => {
      if (editId) {
        ...
        try {
          ...
          console.error('Error fetching export sales order record:', err);
          setMessage('Error loading export sales order record');
  ...
  const handleItemChange = useCallback((index, field, value) => {
    setItems(prevItems => {
      ...
      // Automatic calculations for export order
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result && result.success) {
        setMessage(editId ? 'Export Sales Order updated successfully!' : 'Export Sales Order saved successfully!');
        setMessageType('success');
        setTimeout(() => {
          setMessage('');
          navigate('/entry/sales-export-order-display');
        }, 1000);
      } else {
        setMessage(result?.message || 'Error saving Export Sales Order');
        setMessageType('error');
      }
    } catch (error) {
      console.error('Error saving Export Sales Order:', error);
      setMessage('Error saving Export Sales Order: ' + error.message);
      setMessageType('error');
    } finally {
      setLoading(false);
    }
  ...
  const topFrameFields = [
    // Col 1
    { name: 'billNo', label: 'Order No', type: 'text', readOnly: true, col: 1 },
  ...
  return (
    <div className="window">
      <div className="screen-title">{editId ? 'Edit Export Sales Order' : 'Export Sales Order Creation'}</div>
      ...
      <EntryActions 
        onSave={handleSubmit}
        saving={loading}
        saveText={editId ? "Update Export Sales Order" : "Save Export Sales Order"}
      />
    </div>
  );
};
