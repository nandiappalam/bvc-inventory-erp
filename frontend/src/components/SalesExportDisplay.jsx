...
const columns = [
  ...
  { key: 'sender_name', title: 'Sender' },
  { key: 'consignee_name', title: 'Consignee' },
...
const handlePrint = (row) => {
  const html = `
    <div style="font-family: Arial, sans-serif; padding: 20px;">
      <h2 style="color: #1f4fb2; border-bottom: 2px solid #1f4fb2; padding-bottom: 10px;">Sales Export Details</h2>
      <table style="border-collapse: collapse; width: 100%; margin-top: 15px;">
        ...
        <tr><th style="border: 1px solid #ccc; padding: 10px; text-align: left; background: #1f4fb2; color: white;">Sender</th><td style="border: 1px solid #ccc; padding: 10px;">${row.sender_name||row.sender||''}</td></tr>
        <tr><th style="border: 1px solid #ccc; padding: 10px; text-align: left; background: #1f4fb2; color: white;">Consignee</th><td style="border: 1px solid #ccc; padding: 10px;">${row.consignee_name||row.consignee||''}</td></tr>
  ...
  `;
  printHtml(html, `Sales_Export_${row.bill_no || row.id}`);
};
...
const SalesExportDisplay = () => {
  ...
  const handleEdit = (row) => {
    navigate(`/entry/sales-export-create?id=${row.id}`);
  };
  ...
  return (
    <EntryDisplay
      ...
      apiEndpoint="/api/sales-export-orders?is_order=0"
      columns={columns}
      onEdit={handleEdit}
      onDelete={handleDelete}
      onPrint={handlePrint}
      addNewLink="/entry/sales-export-create"
    />
  );
};
