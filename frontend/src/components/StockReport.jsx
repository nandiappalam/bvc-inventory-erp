...
const StockReport = () => {
  ...
  // Calculate totals
  const getBagWeight = (row) => {
    const rawWeight = parseFloat(row.weight ?? row.per_unit_weight ?? 0);
    const qty = parseFloat(row.purchased_qty || row.total_purchased || row.opening_qty || 1);
    
    // If weight is larger than 120 kg and we have a valid quantity, it is likely a total weight.
    // We divide it by quantity to get the per-bag weight.
    if (rawWeight > 120 && qty > 0) {
      const calcWeight = rawWeight / qty;
      return parseFloat(calcWeight.toFixed(2));
    }
    return rawWeight;
  };
  ...
  const totalWeightBalance = filteredData.reduce((sum, row) => {
    const bagWt = getBagWeight(row);
    if (reportMode === 'summary') {
      return sum + (parseFloat(row.balance || 0) * bagWt);
    } else {
      return sum + (parseFloat(row.remaining_quantity || 0) * bagWt);
    }
  }, 0);
    ...
    <div style={styles.container}>
      ...
      <div style={styles.tabContainer}>
        {['All', 'RM', 'FG', 'Wastage'].map(cat => (
          ...
          >
            {cat === 'All' ? '📌 All Stock' : cat === 'RM' ? '🌾 Raw Material (RM)' : cat === 'FG' ? '📦 Finished Goods (FG)' : '🗑️ Wastage Stock'}
          </button>
        ))}
      </div>
      ...
      {/* Loading */}
      {loading && <div style={styles.loading}>Loading stock & trace registers...</div>}
          ...
          <table style={styles.table}>
            <thead>
              {reportMode === 'summary' ? (
                ...
                <tr>
                  <th style={styles.th} title="Accordion Trigger"></th>
                  <th style={styles.th}>Item Name</th>
                  <th style={styles.th}>Category</th>
                        ...
                        {reportMode === 'summary' ? (
                          <>
                            ...
                            <td style={{
                              ...
                              textAlign: 'right', 
                              fontWeight: 'bold', 
                              color: ((row.balance || 0) * getBagWeight(row)) < 0 ? '#b91c1c' : '#15803d'
                            }}>
                              {((row.balance || 0) * getBagWeight(row)).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kg
                            </td>
                          </>
                        ) : (
                          <>
                            ...
                            <td style={{
                              ...
                              textAlign: 'right', 
                              fontWeight: 'bold', 
                              color: ((row.remaining_quantity || 0) * getBagWeight(row)) <= 0 ? '#94a3b8' : 'green'
                            }}>
                              {((row.remaining_quantity || 0) * getBagWeight(row)).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kg
                            </td>
                            <td style={{...styles.td, textAlign: 'right', color: '#475569'}}>{parseFloat(row.rate || 0).toFixed(2)}</td>
                          </>
                        )}
                      </tr>
      ...
      <div style={styles.totalsWidget}>
        <div style={styles.widgetItem}>
          <span style={styles.widgetLabel}>Total Items Selected:</span>
        ...
        <div style={styles.widgetItem}>
          <span style={styles.widgetLabel}>Total Stock Weight:</span>
          <span style={{...styles.widgetValue, color: '#15803d'}}>{totalWeightBalance.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kg</span>
        </div>
      </div>
      ...
      <div style={styles.footerBar}>
        <div style={styles.footerLeft}>
          <button onClick={handlePrint} style={styles.printBtn}>🖨️ Print Stock Sheet</button>
          <button onClick={handleRefresh} style={styles.actionBtn}>🔄 Force Refresh</button>
        </div>
        <div style={styles.footerRight}>
          Registered Records: {filteredData.length} of {safeData.length}
        </div>
      </div>
...
// Styles matching original HTML format with modern accessibility and visual polish
const styles = {
  container: {
    margin: '0',
