...
export default function DocumentViewerModal({ open, document: doc, onClose, onEdit }) {
  ...
  const handlePrint = () => {
    const el = document.getElementById('controlled-doc-content');
    if (el) {
      printHtml(el.innerHTML, `${doc.doc_number || 'Doc'} - ${doc.title || 'Controlled Document'}`);
    } else {
      window.print();
    }
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent id="controlled-doc-content" sx={{ p: { xs: 2, md: 4 }, backgroundColor: '#ffffff' }}>
