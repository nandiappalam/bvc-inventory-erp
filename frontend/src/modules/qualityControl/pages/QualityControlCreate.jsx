...
export default function QualityControlCreate() {
  ...
  const onPrint = () => {
    // Generate clean printable inspection sheet
    const resultsRows = Object.entries(tableState).flatMap(([cat, rows]) => 
      rows.map(r => `
        <tr>
          ...
          <td style="border: 1px solid #cbd5e1; padding: 6px 10px; text-align: center;">${r.min ?? '-'} - ${r.max ?? '-'}</td>
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; color: #0f172a; padding: 15px;">
        <div style="text-align: center; border-bottom: 2px solid #1e3a8a; padding-bottom: 12px; margin-bottom: 15px;">
          <h2 style="margin: 0; color: #1e3a8a; font-size: 20px;">BVC FOODS QUALITY ASSURANCE DIVISION</h2>
          <p style="margin: 3px 0 0; font-size: 12px; color: #475569; font-weight: 600;">QUALITY CONTROL & LABORATORY TEST INSPECTION REPORT</p>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 6px; padding: 12px; margin-bottom: 15px; font-size: 12px;">
          <div><strong>QC Inspection No:</strong> ${qcHeader.qcId || id || 'Draft'}</div>
          <div><strong>Inspection Date:</strong> ${qcHeader.receiptDate || new Date().toISOString().split('T')[0]}</div>
          <div><strong>Lot Number:</strong> <span style="font-weight: 800; color: #1e3a8a;">${qcHeader.lotNo || '-'}</span></div>
          <div><strong>Item / Product:</strong> ${qcHeader.item || '-'}</div>
          <div><strong>Supplier:</strong> ${qcHeader.supplier || '-'}</div>
          <div><strong>Batch / Ref:</strong> ${qcHeader.batch || '-'}</div>
          <div><strong>Total Quantity:</strong> ${qcHeader.quantity || '-'} bags (${qcHeader.totalWeight || '-'} kg)</div>
          <div><strong>QC Inspector:</strong> ${qcHeader.analyst || 'QC Engineer'}</div>
          <div><strong>Overall Result:</strong> <span style="font-weight: 800; color: ${overallStatus === 'REJECTED' || overallStatus === 'FAIL' ? '#dc2626' : '#16a34a'};">${overallStatus}</span></div>
          <div><strong>Disposition:</strong> ${qcHeader.unloadingStatus || 'Pending'}</div>
        </div>
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 12px; margin-bottom: 20px;">
          <thead>
            <tr style="background: #1e3a8a; color: white;">
              ...
              <th style="border: 1px solid #1e3a8a; padding: 8px 10px; text-align: center;">Standard Limit</th>
              <th style="border: 1px solid #1e3a8a; padding: 8px 10px; text-align: center;">Observed Value</th>
        ...
        ${qcHeader.remarks ? `
          <div style="border: 1px solid #cbd5e1; border-radius: 4px; padding: 10px; margin-bottom: 20px; font-size: 12px; background: #fff;">
            <strong>Quality Remarks & Observations:</strong>
        ...
        <div style="display: flex; justify-content: space-between; margin-top: 40px; padding-top: 20px; border-top: 1px dashed #cbd5e1; font-size: 12px;">
          <div style="text-align: center; width: 180px;">
            ...
            <div style="border-top: 1px solid #0f172a; padding-top: 4px; font-weight: bold;">QC Analyst</div>
          </div>
          <div style="text-align: center; width: 180px;">
            <div style="height: 35px;"></div>
            <div style="border-top: 1px solid #0f172a; padding-top: 4px; font-weight: bold;">QA Manager</div>
          </div>
          <div style="text-align: center; width: 180px;">
            <div style="height: 35px;"></div>
            <div style="border-top: 1px solid #0f172a; padding-top: 4px; font-weight: bold;">Authorized Signatory</div>
          </div>
        </div>
      </div>
    `;

    printHtml(html, `QC Inspection Report - ${qcHeader.lotNo || 'Inspection'}`);
  };
